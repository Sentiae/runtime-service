...
const (
	...
	RuntimeService_Compile_FullMethodName              = "/runtime.v1.RuntimeService/Compile"
)
...
type RuntimeServiceClient interface {
	...
	// ── Multi-file project compile (no execution) ──────────────────────
	// Compile builds a multi-file project in an ephemeral build container
	// and returns structured compiler diagnostics. The built artifact is
	// never executed. Used by codegen-service to compile-verify generated
	// modules. Distinct from the Firecracker execution pool: each call
	// runs a self-contained `docker run --rm` against a toolchain image.
...
func (c *runtimeServiceClient) Compile(ctx context.Context, in *CompileRequest, opts ...grpc.CallOption) (*CompileResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(CompileResponse)
	err := c.cc.Invoke(ctx, RuntimeService_Compile_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (UnimplementedRuntimeServiceServer) Compile(context.Context, *CompileRequest) (*CompileResponse, error) {
...
func _RuntimeService_Compile_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(CompileRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(RuntimeServiceServer).Compile(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: RuntimeService_Compile_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RuntimeServiceServer).Compile(ctx, req.(*CompileRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var RuntimeService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "Compile",
			Handler:    _RuntimeService_Compile_Handler,
		},
	},
