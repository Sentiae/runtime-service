...
6. **Error mapping is SPLIT on purpose (§16.3 deviation):** `internal/app/errors.go`'s `pkerrors.Register` owns only errors defined ELSEWHERE (platform-kit sentinels that would otherwise collapse to `Internal`), while `fleetError`'s hand-map (`handler/grpc/fleet_server.go`) owns every runtime-local `domain.Err*` — its curated messages are load-bearing because `pkerrors.ToGRPC`'s default echoes raw `err.Error()` (DB/Vault text). `fleetError` consults the registry only in its `default` branch, so **registering a hand-mapped sentinel is dead code that looks like a control** — do not add the fleet sentinels to the registry.
